...
function normalizeLink(src) {
  var u, normalized = replaceEntities(src);

  // We don't care much about result of mailformed URIs,
  // but shoud not throw exception.
  try {
    u = url.parse(normalized, false, true);

    // u.auth is decoded/encoded automatically
    // u.host/u.hostname are decoded/encoded automatically.
    if (u.pathname) { u.pathname = encodeURI(decodeURI(u.pathname)); }
    if (u.search) { u.search = encodeURI(decodeURI(u.search.replace(/^\?/, ''))); }
    if (u.hash) { u.hash = encodeURI(decodeURI(u.hash.replace(/^\?/, ''))); }
    return url.format(u);
  } catch (__) {
    return '';
  }
