...
rules.code_block = function (tokens, idx /*, options, env */) {
  return '<pre><code>' + escapeHtml(tokens[idx].d.content) + '</code></pre>\n';
};
rules.code_inline = function (tokens, idx /*, options, env */) {
  return '<code>' + escapeHtml(tokens[idx].d.content) + '</code>';
};
...
rules.fence = function (tokens, idx, options /*, env, self*/) {
  ...
  if (token.d.params) {
    langName = escapeHtml(replaceEntities(unescapeMd(token.d.params.split(/\s+/g)[0])));
    langClass = ' class="' + langPrefix + langName + '"';
  }

  if (options.highlight) {
    highlighted = options.highlight(token.d.content, langName) || escapeHtml(token.d.content);
  } else {
    highlighted = escapeHtml(token.d.content);
  }
...
rules.list_item_open     = function (tokens, idx /*, options, env */) {
  ...
  if ((next.type === 'list_item_close') ||
      (next.type === 'paragraph_open' && next.d.tight)) {
    return '<li>';
  }
  return '<li>\n';
};
...
rules.ordered_list_open  = function (tokens, idx /*, options, env */) {
  if (tokens[idx].d.order > 1) {
    return '<ol start="' + tokens[idx].d.order + '">\n';
  }
  return '<ol>\n';
};
...
rules.paragraph_close = function (tokens, idx /*, options, env */) {
  ...
  //
  if (tokens[idx].d.tight === true) {
    if (!tokens[idx - 1].d.content) {
      return '';
    }
...
rules.link_open = function (tokens, idx /*, options, env */) {
  var title = tokens[idx].d.title ? (' title="' + escapeHtml(replaceEntities(tokens[idx].d.title)) + '"') : '';
  var target = tokens[idx].d.target ? (' target="' + escapeHtml(tokens[idx].d.target) + '"') : '';
  return '<a href="' + escapeHtml(tokens[idx].d.href) + '"' + title + target + '>';
};
...
rules.image = function (tokens, idx, options, env, self) {
  var src = ' src="' + escapeHtml(tokens[idx].d.src) + '"';
  var title = tokens[idx].d.title ? (' title="' + escapeHtml(replaceEntities(tokens[idx].d.title)) + '"') : '';
  var alt = ' alt="' + self.renderInlineAsText(tokens[idx].d.tokens, options, env) + '"';
...
rules.th_open     = function (tokens, idx /*, options, env */) {
  if (tokens[idx].d.align) {
    return '<th style="text-align:' + tokens[idx].d.align + '">';
  }
  return '<th>';
};
...
rules.td_open     = function (tokens, idx /*, options, env */) {
  if (tokens[idx].d.align) {
    return '<td style="text-align:' + tokens[idx].d.align + '">';
  }
  return '<td>';
};
...
rules.text = function (tokens, idx /*, options, env */) {
  return escapeHtml(tokens[idx].d.content);
};


rules.html_block = function (tokens, idx /*, options, env */) {
  return tokens[idx].d.content;
};
...
Renderer.prototype.renderInlineAsText = function (tokens, options, env) {
  ...
  for (var i = 0, len = tokens.length; i < len; i++) {
    if (tokens[i].type === 'text') {
      ...
      result += this.renderInlineAsText(tokens[i].d.tokens, options, env);
    }
  }
...
Renderer.prototype.render = function (tokens, options, env) {
  ...
  for (i = 0, len = tokens.length; i < len; i++) {
    if (tokens[i].type === 'inline') {
      result += this.renderInline(tokens[i].d.children, options, env);
    } else {
      result += _rules[tokens[i].type](tokens, i, options, env, this);
    }
  }
